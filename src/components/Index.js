...
const getUsers = async () => {
  const allUsers = await fetch(
    `${process.env.REACT_APP_QCHALLENGE_API_URI}/all-users`
  );
  const AllusersFormated = await allUsers.json();
  const allUsersCrypted = CryptoJS.AES.encrypt(
    JSON.stringify(AllusersFormated),
...
const Index = () => {
  ...
  const [users] = useState(() => {
    const usersDecrypted = CryptoJS.AES.decrypt(
  ...
  useEffect(() => {
    const initialGetMessages = CryptoJS.AES.decrypt(
      getCryptedMessages,
      `${process.env.REACT_APP_CRYPTO_KEY}`
    ).toString(CryptoJS.enc.Utf8);
  ...
  useEffect(() => {
    ...
    socket.on("request-join-duel", ({ joinDuelID, senderID, senderPseudo }) => {
      ...
      if (joinDuelID === localStorage.getItem("duelID")) {
        ...
        localStorage.setItem(
          "duel-gamer",
          JSON.stringify({ creator: duelCreator, joiner: senderID })
        );
    ...
    socket.on(
      "joined-duel",
      ({ duelLevel, senderID, duelCreator, duelTheme }) => {
        ...
        const joinerPseudo = JSON.parse(localStorage.getItem("user")).data
          .pseudo;
        socket.emit("joiner", {
          joinerPseudo,
          duelCreator,
  ...
  const stopDuel = () => {
    ...
    if (duelJoiner !== undefined) {
      ...
      if (duelStoperObj !== undefined) {
        const duelStoper = duelStoperObj.pseudo;
        socket.emit("stop-duel", duelStoper);
      }
      setNewLeftDuelPosition("unset");
      setNewRightDuelPosition("50px");
      setShowDuelDetails(false);
  ...
  return (
    <>
      ...
      <Header
        ...
        isGameDuel={isDuel}
      />
      {isDuel && showDuelDetails && !isSearch && (
        <DuelDetails
          duelID={duelID}
      ...
      {isDuel && showDuelInputID && !isSearch && (
        <InputDuelID
      ...
      <Gamespace
        ...
        research={isSearch}
      />
      {<DiscutionIcon openChatWindow={openChatWindow} />}

      {openDiscution && (
        <Chat
          closeChatWindow={closeChatWindow}
          openChat={(e) => {
            const getMessages = CryptoJS.AES.decrypt(
              getCryptedMessages,
              `${process.env.REACT_APP_CRYPTO_KEY}`
            ).toString(CryptoJS.enc.Utf8);
            ...
            const receiver = users.find(
              (data) => data.pseudo === reciverPseudo.current
            );
