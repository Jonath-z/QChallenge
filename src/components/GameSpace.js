...
const Gamespace = (props) => {
  ...
  const [phoneMediaShowGamewindowDiv, setPhoneMediaShowGamewindowDiv] =
    useState(false);
  // *************************** FETCH INITAL DATA (THEME AND QUESTIONS) ****************************************/
  useEffect(() => {
    async function data() {
      const data = await fetch(
        `${process.env.REACT_APP_QCHALLENGE_API_URI}/theme`
      );
  ...
  useEffect(() => {
    if (!props.isGameDuel) {
      ...
      if (userData.data.score !== undefined) {
        ...
        score.current = userData.data.score.find(
          ({ theme }) => theme === challengeTheme
        ).score;
        scoreIncrement.current = 5;
        questionIndex.current = userData.data.score.find(
          ({ theme }) => theme === challengeTheme
        ).level;
      }
  ...
  useEffect(() => {
    if (challengeTheme !== null && questions.current !== null) {
      const currentQuestions = questions.current.filter(
        (question) => question.theme === `${challengeTheme}`
      );
      const currentQuestionsWithCity = currentQuestions.filter(
        (question) => question.city !== null
      );
  ...
  const generateTheCurrentQuestion = () => {
    if (questionsInCurrentTheme !== "undefined") {
      ...
      const myQuestions =
        questionsInCurrentTheme.current[questionIndex.current];
      ...
      if (challengeTheme === "Country and Capital") {
        const answeroptions = generateOptionForContryAndCapital(
          myQuestions.city,
          questionsInCurrentTheme.current,
          contry
        );
        ...
        const options = questionsInCurrentTheme.current.map(
          (question) => question.options
        );
        // console.log(options)
        const currentQuestion = generateQuestionOtherThanCountryAndCapital(
          questionsInCurrentTheme.current,
          questionIndex.current
        );
        setOtherQuestion(currentQuestion);
        // console.log(currentQuestion);
        const currentOptions = generateOptionsOtherThanCountryAndCapital(
          options,
          questionIndex.current
        );
  ...
  const getAnswer = (e) => {
    ...
    if (answerState === true) {
      ...
      progressBar.current = userChallengeProgress(
        questionIndex.current,
        questionsInCurrentTheme.current
      );
  ...
  const openChallenge = (e) => {
    ...
    if (prevTheme !== e.target.innerHTML && !props.showStartButton) {
      ...
      const confirmResponse = window.confirm(
        "Do want to leave the challenge ?"
      );
      if (confirmResponse) {
      ...
      // console.log(userScoreState);
      score.current = userData.data.score.find(
        ({ theme }) => theme === e.target.innerHTML
      ).score;
      questionIndex.current = userData.data.score.find(
        ({ theme }) => theme === e.target.innerHTML
      ).level;
  ...
  return (
    <>
      ...
      {!phoneMediaShowGamewindowDiv && (
        <MediaQuery minWidth={300} maxWidth={414}>
          {!props.isGameDuel && !props.research && (
      ...
      {(phoneMediaShowGamewindowDiv || props.isGameDuel) && (
        <MediaQuery minWidth={300} maxWidth={414}>
          <div
            className="gameWindowDiv"
            style={{
              left: props.NewLeftPosition,
              right: props.NewRightPostion,
            }}
          >
            {!props.isGameDuel && !props.duelSettingReady && (
              <ScoreBar
                ...
                myThemes ? (
                  myThemes.map((theme) => (
                    <div className="themeContainer" key={uuid()}>
                      <p className="theme" onClick={openChallenge}>
                        {theme.theme}
                      </p>
                    </div>
                  ))
