...
const Account = () => {
  ...
  useEffect(() => {
    const getUser = async () => {
      await fetch(`${process.env.REACT_APP_QCHALLENGE_API_URI}/get-user-data`, {
        method: "POST",
        headers: {
          accept: "*/*",
    ...
    uploadTask.on(
      ...
      () => {
        uploadTask.snapshot.ref.getDownloadURL().then((url) => {
          ...
          setUserProfile(JSON.parse(localStorage.getItem("user")));
          fetch(
            `${process.env.REACT_APP_QCHALLENGE_API_URI}/upgate-profile/image`,
            {
              method: "POST",
              headers: {
                accept: "*/*",
  ...
  const updateProfileDetails = () => {
    const update = async () => {
      await fetch(
        `${process.env.REACT_APP_QCHALLENGE_API_URI}/update-user-datails`,
        {
          method: "POST",
          headers: {
            accept: "*/*",
  ...
  return (
    <div className="account">
        ...
        <div className="user-profile-container">
          <div className="profile-image-container">
            ...
            <div className="plus-icon-container">
              <BsPlusCircleFill className="plus-icon" onClick={openFolder} />
            </div>
          </div>
          <div className="profile-details-container">
            <p className="pseudo" onClick={openInputPseudo}>
              Pseudo: {JSON.parse(localStorage.getItem("user")).data.pseudo}
            </p>
            <input
              type="text"
              placeholder="New pseudo"
              className="inputNewPseudo"
              onChange={getNewPseudo}
              style={{
                display: showInputPseudo,
              }}
            />
            <p className="email" onClick={openInputEmail}>
              Email: {userProfileEmail}
            </p>
            <input
              type="email"
              placeholder="New email"
              className="inputNewEmail"
      ...
      {showUpdateButton && (
        <div>
          <button className="update-button" onClick={updateProfileDetails}>
            Update
          </button>
        </div>
      )}
      ...
      <div>
        <button
          className="update-button logout-button"
          onClick={() => {
