...
const DiscussionWindow = (props) => {
  ...
  const setBackgroungColor = (e) => {
    ...
    const updateFontColor = async () => {
      await fetch(
        `${process.env.REACT_APP_QCHALLENGE_API_URI}/update-font-color`,
        {
          method: "POST",
          headers: {
            accept: "*/*",
  ...
  const moveToTrash = () => {
    if (messagesForTrash !== null) {
      ...
      const decryptMessages = JSON.parse(
      ...
      let newMessages = decryptMessages.filter(
        ({ message }) => message !== messagesForTrash
      );
  ...
  return (
    <>
      <div
        className="discussion-window"
        style={{ background: DiscussionBacgroungColor }}
      >
        <div className="receiver-details">
          <div className="receiver-details-profile">
            <FaArrowCircleLeft
              onClick={props.closeDiscussionWindow}
              className="faArrow-left"
            />
            <img
              src={props.receiverAvatar}
              alt="profile"
              className="receiver-details-profile-avatar"
            />
            <p className="receiver-details-profile-pseudo">
              {props.receiverPseudo}
            </p>
          </div>
          <p className="drop-down-option">
            <FiMenu
              className="menue receiver-details-icon"
              style={{ display: showMenu }}
            />
            <FaTrash
              className="trash receiver-details-icon"
              style={{ display: showTrash }}
              onClick={moveToTrash}
            />
            {isMenue && (
              <span className="backgroung-color-menu">
                <span>
                  <li className="color-option" onClick={setBackgroungColor}>
                    White
                  </li>
                  <li className="color-option" onClick={setBackgroungColor}>
                    Turquoise
        ...
        <div className="message-container">
          <ScrollableFeed className="message-scollableFeed">
            {props.allMessages.map((message, i) => {
              if (
                message.sender === props.senderID &&
                message.receiver === props.receiverID
              ) {
                return (
                  <p
                  ...
                  >
                    {message.message}{" "}
                    <span className="time-outCome">{message.time}</span>
                  </p>
                );
              } else if (
                message.receiver === props.senderID &&
                message.sender === props.receiverID
              ) {
                return (
                  <p
                  ...
                  >
                    {message.message}{" "}
                    <span className="time-outIncome">{message.time}</span>
                  </p>
                );
              }
              return "";
        ...
        >
          ...
          />
          <IoMdSend
            className="input-container-send-incon"
