...
const Signup = () => {
    ...
    if (avatar !== null) {
      const fetchData = async () => {
        ...
        const data = await fetch(
          `${process.env.REACT_APP_QCHALLENGE_API_URI}/signup`,
          {
            method: "POST",
            headers: {
              accept: "*/*",
        ...
        if (response === "error") {
          setLoginErr(
            "something went wrong,(your password must have 4 characters minimum)"
          );
  ...
  return (
    <>
      {showSignup && (
        <div className="loginContainer signup-div">
          <div className="login">
            ...
            <button onClick={registerUser} className="loginBtn">
              Submit
            </button>
            <p
              className="singup-para"
              onClick={() => {
                setShowLogin(true);
                setShowSignup(false);
              }}
