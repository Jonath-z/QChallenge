...
const Research = (props) => {
  ...
  const goToUserProfile = (e) => {
    ...
    const user = allUsers.filter(
      ({ pseudo, avatar }) => pseudo === userPseudo && avatar === userProfileUrl
    );
  ...
  const goToChat = (e) => {
    ...
    const userProfileUrl =
      e.target.parentNode.parentNode.previousSibling.previousSibling.src;
    ...
    <>
      ...
      >
        <div className="cancel">
          <MdCancel className="cancel" onClick={closeIsResearch} />
        </div>
        {getAllUsers()}
      </div>
      {showProfile && (
